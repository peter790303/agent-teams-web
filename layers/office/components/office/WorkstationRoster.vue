...
<template><footer class="roster"><div v-for="person in peopleWithStatus" :key="person.role" class="employee-card" :class="{ 'is-active': person.active }"><div class="employee-avatar"><span class="avatar-hair" /><span class="avatar-face" /></div><div><strong>{{ person.name }}</strong><small>{{ person.role }}</small><em>{{ person.status }}</em><i v-if="person.active" /></div></div><button class="add-employee" type="button" aria-label="新增員工">＋</button></footer></template>
