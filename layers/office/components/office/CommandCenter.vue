...
<template>
  <aside class="panel command-center" aria-label="Command Center">
    <h2 class="panel-title ma-0 px-3 py-2 text-subtitle-2 font-weight-bold">COMMAND CENTER</h2>
    <v-tabs
      v-model="tab"
      class="command-tabs"
      aria-label="指揮中心分頁"
    ...
    <template v-if="tab === 'overview'">
      <section class="command-section px-3 py-2 border-b">
        ...
        <div class="feed mt-1">
          ...
          >
            <time class="flex-shrink-0 text-caption text-medium-emphasis">資料未提供</time>
            <p class="ma-0 text-caption text-medium-emphasis">{{ item }}</p>
          </div>
          <div v-if="!feed.length" class="empty d-flex flex-column ga-1 py-1 text-caption text-medium-emphasis">
      ...
      <section class="command-section px-3 py-2 border-b">
        ...
        <div v-if="statsLabel" class="notice mt-1 text-caption text-medium-emphasis">{{ statsLabel }}</div>
        <div v-else class="stat-grid mt-2">
          <div class="pa-1 text-center border">
            <strong class="d-block text-subtitle-2 text-warning">{{ stats.completed }}</strong
            ><small class="text-caption text-medium-emphasis">完成任務</small>
          </div>
          <div class="pa-1 text-center border">
            <strong class="d-block text-subtitle-2 text-warning">{{ stats.waiting }}</strong
            ><small class="text-caption text-medium-emphasis">等待處理</small>
          </div>
          <div class="pa-1 text-center border">
            <strong class="d-block text-subtitle-2 text-warning">{{ stats.failed }}</strong
            ><small class="text-caption text-medium-emphasis">錯誤</small>
          </div>
          <div class="pa-1 text-center border">
            <strong class="d-block text-subtitle-2 text-warning">{{ stats.running }}</strong
            ><small class="text-caption text-medium-emphasis">運行中</small>
          </div>
        </div>
      </section>
      <section class="command-section px-3 py-2 border-b">
        ...
        <div class="task-list d-flex flex-column ga-1 mt-1">
          ...
          <p v-if="!pendingTasks.length" class="empty ma-0 py-1 text-caption text-medium-emphasis">
            目前沒有待處理任務
          </p>
        </div>
      </section>
      <section class="command-section px-3 py-2 border-b">
        ...
        <p class="notice ma-0 mt-1 text-caption text-medium-emphasis">資料未提供</p>
      </section>
    </template>
    <section v-else-if="tab === 'activity'" class="command-section px-3 py-2 border-b">
      <div class="heading-row d-flex align-center justify-space-between">
    ...
    <section v-else class="command-section px-3 py-2 border-b">
      <div class="heading-row d-flex align-center justify-space-between">
      ...
      <div class="task-list d-flex flex-column ga-1 mt-1">
        ...
        >
        <div v-if="!props.tasks.length" class="empty py-1 text-caption text-medium-emphasis">目前沒有任務</div>
      </div>
    </section>
  </aside>
</template>
