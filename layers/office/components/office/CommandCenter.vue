...
const feed = computed(() => props.activities.length ? props.activities : props.tasks.slice(0, 5).map((task) => `${task.purpose} · ${task.stage}`))
</script>
<template>
  <aside class="panel command-center" aria-label="Command Center">
    ...
    <nav class="tabs command-tabs" aria-label="指揮中心分頁">
      <button :aria-selected="tab === 'overview'" @click="tab = 'overview'"><b>◈</b>總覽</button>
      <button :aria-selected="tab === 'tasks'" @click="tab = 'tasks'"><b>✓</b>任務</button>
      <button :aria-selected="tab === 'activity'" @click="tab = 'activity'"><b>▣</b>動態</button>
    ...
    <section v-else class="command-section"><div class="heading-row"><h2>任務清單</h2><button class="soft-link" @click="tab = 'overview'">返回總覽</button></div><div class="task-list"><NuxtLink v-for="task in props.tasks" :key="task.id" class="task-item" :to="`/office/tasks/${task.id}`"><strong>{{ task.purpose }}</strong><small>{{ task.projectId }} · {{ task.stage }}</small></NuxtLink><div v-if="!props.tasks.length" class="empty">目前沒有任務</div></div></section>
  </aside>
</template>
