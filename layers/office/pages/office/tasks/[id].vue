...
<template>
  <BasePageLayout>
    <main class="panel-page">
      <NuxtLink to="/">← 返回 AI Office</NuxtLink><p v-if="error" role="alert">{{ error }}</p>
      <template v-if="state"><h1>Office 工作詳情</h1><p class="purpose">{{ state.data.task.purpose }}</p><div class="badge">{{ state.data.stage }} · {{ state.data.activity }}</div>
        <v-row><v-col cols="12"><section class="detail-grid"><article><h2>角色與階段</h2><p v-for="line in dispatchLines" :key="line">{{ line }}</p></article><article><h2>執行摘要</h2><p v-for="line in summary" :key="line">{{ line }}</p></article><article><h2>阻塞與 QA</h2><p v-for="blocker in blockers" :key="blocker">⚠️ {{ blocker }}</p><div><strong>QA 報告</strong><p v-if="!hasQaRows">尚未產生</p><dl v-else><template v-for="row in qaRows" :key="row.label"><dt>{{ row.label }}</dt><dd>{{ row.value }}</dd></template></dl></div><p>交付：{{ state.data.delivery?.status ?? '尚未建立' }}</p></article></section></v-col></v-row>
        <v-row><v-col cols="12"><section><h2>人工接手</h2><p>狀態：{{ interventionData?.status ?? '尚未請求人工接手' }}</p><p>角色：{{ interventionData?.role ?? '未知' }} · 回合：{{ interventionData?.round ?? '未知' }}</p><p>Branch：{{ interventionData?.branch ?? '未知' }} · Worktree：{{ displayWorktree }}</p><p>Diagnostics：{{ diagnosticsText }}</p><fieldset><label>接手指令 <input v-model="instruction" :disabled="!interventionEditable" placeholder="接手指令" ></label><label>Worktree <input v-model="worktree" :disabled="!interventionEditable" placeholder="worktree" ></label><label>失敗 review 回合 <input v-model.number="failedReviewRounds" :disabled="!interventionEditable" min="0" type="number" ></label><label>額外 review allowance <input v-model="extraReviewRoundAllowance" :disabled="!interventionEditable" placeholder="需要時填寫授權" ></label><button :disabled="!interventionEditable" @click="resume">Resume</button><label>Commit <input v-model="commit" :disabled="!interventionEditable" placeholder="commit" ></label><label>Gate <select v-model="gate" :disabled="!interventionEditable"><option disabled value="">選擇 gate</option><option v-for="gateOption in gateOptions" :key="gateOption" :value="gateOption">{{ gateOption }}</option></select></label><button :disabled="submitDisabled" @click="submit">Submit</button></fieldset><p v-if="!interventionEditable">目前 intervention 不可編輯，操作已停用。</p><p v-if="status" role="status">{{ status }}</p></section></v-col></v-row>
      </template><p v-else>載入中…</p>
    </main>
  </BasePageLayout>
</template>
