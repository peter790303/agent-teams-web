...
<template>
  <main class="panel-page"><NuxtLink to="/">← 返回 AI Office</NuxtLink><h1>模型設定與監控</h1><p>管理角色白名單、候選模型能力、評分、容量與 Provider 健康狀態。</p>
    ...
    <section v-for="editor in editors" :key="editor.role" class="policy"><h2>{{ editor.role }}</h2><p v-if="editor.loadError">{{ editor.loadError }}</p><p v-if="editor.validationError" role="alert">{{ editor.validationError }}</p>
