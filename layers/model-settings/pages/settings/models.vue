...
<template>
  <main class="panel-page"><NuxtLink to="/">← 返回 AI Office</NuxtLink><h1>模型設定與監控</h1><p>管理角色白名單、候選模型能力、評分、容量與 Provider 健康狀態。</p>
    ...
    <section v-for="editor in editors" :key="editor.role" class="policy"><h2>{{ editor.role }}</h2><p v-if="editor.loadError">{{ editor.loadError }}</p>
      <label>最低品質分數 <input v-model.number="editor.minQualityScore" type="number" min="0" /></label>
      <ul><li v-for="candidate in editor.candidates" :key="id(candidate)"><label><input :checked="editor.selectedIds.includes(id(candidate))" type="checkbox" @change="toggle(editor, candidate)" /> {{ candidate.providerId }}/{{ candidate.modelId }}</label>
        <label>能力 <input v-model="candidate.capabilities" :aria-label="`${candidate.modelId} capabilities`" /></label><label>品質 <input v-model.number="candidate.qualityScore" type="number" /></label><label>成本 <input v-model.number="candidate.costScore" type="number" /></label><label>延遲 <input v-model.number="candidate.latencyScore" type="number" /></label>
      </li></ul>
      <form @submit.prevent="addCandidate(editor)"><input v-model="editor.draft.providerId" aria-label="Provider" placeholder="Provider" /><input v-model="editor.draft.modelId" aria-label="Model" placeholder="Model" /><input v-model="editor.draft.capabilities" aria-label="Capabilities" placeholder="Capabilities" /><button type="submit">新增候選</button></form><button @click="save(editor)">儲存</button>
    </section>
