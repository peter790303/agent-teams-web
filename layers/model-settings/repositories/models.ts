...
import type { CapacityResource, HealthResource, PolicyResource, SavePolicyPayload } from '../types/api'
...
const assemblePolicy = (response: PolicyResource, fallbackRole: Role): Policy => {
...
export const getPolicy = ({ role }: GetPolicyInput): Promise<Policy> => request<PolicyResource>(`/model-policies/${role}`).then((response) => assemblePolicy(response, role))
export interface SavePolicyInput { role: Role; whitelist: ModelCandidate[]; minQualityScore?: number }
export const savePolicy = ({ role, whitelist, minQualityScore = 0 }: SavePolicyInput): Promise<Policy> => { const body: SavePolicyPayload = { minQualityScore, whitelist }; return request<PolicyResource>(`/model-policies/${role}`, { method: 'PUT', body }).then((response) => assemblePolicy(response, role)) }
...
export const getHealth = (): Promise<HealthEntry[]> => request<HealthResource>('/provider-health').then((response) => (response.data ?? []).map(assembleHealth))
export const getCapacities = (): Promise<Capacity[]> => request<CapacityResource>('/roles/capacities').then((response) => (response.data ?? []).map(assembleCapacity))
